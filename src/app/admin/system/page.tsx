...
export default function AdminSystemHealthPage() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated) {
        router.push('/login?redirect_url=/admin/system');
  ...
  if (loading || !isAuthorized) {
    return (
      <div className="flex flex-col min-h-screen bg-secondary/30">
        <AdminSubPageHeader title="System Health" />
        <main className="flex-grow flex flex-col items-center justify-center p-4">
          {loading && (
            <>
              <Loader2 className="h-12 w-12 animate-spin text-primary" />
              <p className="mt-2 text-lg text-muted-foreground">Loading System Health...</p>
            </>
          )}
        ...
        <Card className="shadow-xl bg-card/95 backdrop-blur-sm">
            <CardHeader>
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <CardTitle className="font-headline text-3xl text-primary">System Health & Settings</CardTitle>
                        <CardDescription>Monitor system status, configure settings, and manage integrations.</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <div className="min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-muted-foreground/30 rounded-lg p-8 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-muted-foreground mb-2">System Monitoring & Configuration Tools Coming Soon</h3>
                    <p className="text-sm text-muted-foreground">
                        This area will provide insights into system performance and options for platform configuration.
                    </p>
