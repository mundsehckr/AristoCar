...
export default function AdminDisputeResolutionPage() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated) {
        router.push('/login?redirect_url=/admin/disputes');
  ...
  if (loading || !isAuthorized) {
    return (
      <div className="flex flex-col min-h-screen bg-secondary/30">
        <AdminSubPageHeader title="Dispute Resolution" />
        <main className="flex-grow flex flex-col items-center justify-center p-4">
          {loading && (
            <>
              <Loader2 className="h-12 w-12 animate-spin text-primary" />
              <p className="mt-2 text-lg text-muted-foreground">Loading Dispute Resolution...</p>
            </>
          )}
        ...
        <Card className="shadow-xl bg-card/95 backdrop-blur-sm">
            <CardHeader>
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <CardTitle className="font-headline text-3xl text-primary">Dispute Resolution Center</CardTitle>
                        <CardDescription>Manage and resolve user-reported disputes and issues.</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <div className="min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-muted-foreground/30 rounded-lg p-8 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-muted-foreground mb-2">Dispute Management Tools Coming Soon</h3>
                    <p className="text-sm text-muted-foreground">
                        This section will allow administrators to review, track, and mediate disputes.
                    </p>
